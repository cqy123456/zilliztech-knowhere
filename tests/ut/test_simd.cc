...
TEST_CASE("Test minhash function") {
    // only support ref and avx512 by far
    ...
    constexpr size_t seed = 111;
     ...
     SECTION("test minhash distance") {
       ...
       CHECK_EQ(faiss::u32_jaccard_distance((const char*)u32_x.get(), (const char*)u32_y.get(), dim, 4), faiss::u32_jaccard_distance_ref((const char*)u32_x.get(),(const char*)u32_y.get(), dim, 4));
        faiss::u32_jaccard_distance_batch_4((const char*)(&u32_y[0]), (const char*)(&u32_x[0]),  (const char*)(&u32_x[1]), (const char*)(&u32_x[2]),(const char*)(&u32_x[3]), dim, 4, res_dis[0], res_dis[1], res_dis[2], res_dis[3]);
        faiss::u32_jaccard_distance_batch_4_ref((const char*)(&u32_y[0]), (const char*)(&u32_x[0]),  (const char*)(&u32_x[1]), (const char*)(&u32_x[2]),(const char*)(&u32_x[3]), dim, 4, gt_ids[0], gt_ids[1], gt_ids[2], gt_ids[3]);
        CHECK_EQ(res_dis[0], gt_ids[0]);
        CHECK_EQ(res_dis[1], gt_ids[1]);
        CHECK_EQ(res_dis[2], gt_ids[2]);
        CHECK_EQ(res_dis[3], gt_ids[3]);
        faiss::u64_jaccard_distance_batch_4((const char*)(&u64_y[0]), (const char*)(&u64_x[0]),  (const char*)(&u64_x[1]), (const char*)(&u64_x[2]),(const char*)(&u64_x[3]), dim, 8, res_dis[0], res_dis[1], res_dis[2], res_dis[3]);
        faiss::u64_jaccard_distance_batch_4_ref((const char*)(&u64_y[0]), (const char*)(&u64_x[0]),  (const char*)(&u64_x[1]), (const char*)(&u64_x[2]),(const char*)(&u64_x[3]), dim, 8, gt_ids[0], gt_ids[1], gt_ids[2], gt_ids[3]);
        CHECK_EQ(res_dis[0], gt_ids[0]);
        CHECK_EQ(res_dis[1], gt_ids[1]);
        CHECK_EQ(res_dis[2], gt_ids[2]);
        CHECK_EQ(res_dis[3], gt_ids[3]);
