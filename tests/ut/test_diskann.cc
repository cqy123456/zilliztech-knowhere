...
template <typename T>
void
WriteRawDataToDisk(const std::string data_path, const T* raw_data, const uint32_t num, const uint32_t dim) {
    ...
    writer.write((char*)raw_data, sizeof(T) * num * dim);
    writer.close();
}
...
inline void
base_search() {
    std::cout << "test data type" << typeid(data_type).name() << std::endl;
    fs::remove_all(kDir);
    fs::remove(kDir);
    REQUIRE_NOTHROW(fs::create_directory(kDir));
    REQUIRE_NOTHROW(fs::create_directory(kL2IndexDir));
    ...
    if (!std::is_same_v<float, data_type>) {
        base_ds = knowhere::data_type_conversion<float, data_type>(*fp32_base_ds);
        query_ds = knowhere::data_type_conversion<float, data_type>(*fp32_query_ds);
    }
    {
        auto base_ptr = static_cast<const data_type*>(base_ds->GetTensor());
        WriteRawDataToDisk<data_type>(kRawDataPath, base_ptr, kNumRows, kDim);

        // generate the gt of knn search and range search
        auto base_json = base_gen();

        auto result_knn = knowhere::BruteForce::Search<data_type>(base_ds, query_ds, base_json, nullptr);
        knn_gt_ptr = result_knn.value();
        auto result_range = knowhere::BruteForce::RangeSearch<data_type>(base_ds, query_ds, base_json, nullptr);
    ...
    SECTION("Test search and range search") {
        ...
        {
            knowhere::DataSet* ds_ptr = nullptr;
            auto diskann = knowhere::IndexFactory::Instance().Create<data_type>("DISKANN", version, diskann_index_pack);
        ...
        }
        {
            ...
            // knn search without cache file
            {
                ...
                auto diskann_tmp =
                    knowhere::IndexFactory::Instance().Create<data_type>("DISKANN", version, diskann_index_pack);
            ...
            for (const float threshold : bitset_thresholds) {
                ...
                for (const float percentage : bitset_percentages) {
                    for (const auto& gen_func : gen_bitset_funcs) {
                        ...
                        auto gt = knowhere::BruteForce::Search<data_type>(base_ds, query_ds, knn_json, bitset);
...
TEST_CASE("Test DiskANNIndexNode.", "[diskann]") {
    base_search<knowhere::fp16>();
    base_search<knowhere::bf16>();
