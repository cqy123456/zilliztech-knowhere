...
namespace knowhere {
// TODO: Remove template FileDataType after supporting fp16/bf16.
template <typename DataType, typename FileDataType = DataType>
...
Status
DiskANNIndexNode<DataType, FileDataType>::Build(const DataSet& dataset, const Config& cfg) {
    ...
    if constexpr (!std::is_same_v<FileDataType, DataType>) {
        std::string mock_data_path = data_path + "_mock";
        diskann::convert_types_in_file<FileDataType, DataType>(data_path, mock_data_path);
    ...
    if constexpr (!std::is_same_v<DataType, FileDataType>) {
...
template <typename DataType, typename FileDataType>
Status
DiskANNIndexNode<DataType, FileDataType>::Deserialize(const BinarySet& binset, const Config& cfg) {
    std::lock_guard<std::mutex> lock(preparation_lock_);
...
template <typename DataType, typename FileDataType>
expected<DataSetPtr>
DiskANNIndexNode<DataType, FileDataType>::Search(const DataSet& dataset, const Config& cfg,
...
template <typename DataType, typename FileDataType>
expected<DataSetPtr>
DiskANNIndexNode<DataType, FileDataType>::RangeSearch(const DataSet& dataset, const Config& cfg,
                                                      const BitsetView& bitset) const {
    if (!is_prepared_.load() || !pq_flash_index_) {
        LOG_KNOWHERE_ERROR_ << "Failed to load diskann.";
...
template <typename DataType, typename FileDataType>
expected<DataSetPtr>
DiskANNIndexNode<DataType, FileDataType>::GetVectorByIds(const DataSet& dataset) const {
    if (!is_prepared_.load() || !pq_flash_index_) {
        LOG_KNOWHERE_ERROR_ << "Failed to load diskann.";
        return expected<DataSetPtr>::Err(Status::empty_index, "index not loaded");
    }
...
template <typename DataType, typename FileDataType>
expected<DataSetPtr>
DiskANNIndexNode<DataType, FileDataType>::GetIndexMeta(const Config& cfg) const {
...
template <typename DataType, typename FileDataType>
uint64_t
DiskANNIndexNode<DataType, FileDataType>::GetCachedNodeNum(const float cache_dram_budget, const uint64_t data_dim,
...
KNOWHERE_MOCK_REGISTER_GLOBAL(DISKANN, DiskANNIndexNode, fp16, fp32, fp16);
KNOWHERE_MOCK_REGISTER_GLOBAL(DISKANN, DiskANNIndexNode, bf16, fp32, bf16);
}  // namespace knowhere
