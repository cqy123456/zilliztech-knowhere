...
GenFp32TrainDataSet(const DataSetPtr& src, bool is_cosine = false) {
    ...
    if (rows <= kMaxTrainSize) {
        ...
        auto dim = src->GetDim();
        const DataType* src_data = src->GetTensor();
...
class OffsetIndexNode : public IndexNode {
 ...
 public:
    OffsetIndexNode(const ViewDataOp& view_raw_data) : view_data_op_(view_raw_data) {}

    Status
    Train(const DataSetPtr dataset, std::shared_ptr<Config> cfg) override {
        const BaseConfig& base_cfg = dynamic_cast<const BaseConfig>(*cfg);
        ...
        quant_index_ = std::shared_ptr<IndexNode>();
        auto fp32_train_ds = GenFp32TrainDataSet<DataType>(dataset);
        quant_index_->Train(fp32_train_ds, cfg);
    }
    ...
    Search(const DataSetPtr dataset, std::unique_ptr<Config> cfg, const BitsetView& bitset) const override {
        if (this->quant_index_ == nullptr|| this->refine_offset_index_ == nullptr) {
            LOG_KNOWHERE_WARNING_ << "search on empty index";
            return expected<DataSetPtr>::Err(Status::empty_index, "index not is trained.");
        }  
        MatchQuantIndexConfig(cfg.get());
        const BaseOffsetIndexRefineConfig& base_cfg = static_cast<const BaseOffsetIndexRefineConfig&>(*cfg);
        auto nq = dataset->GetRows();
        auto topk = base_cfg.k.value();
        auto refine_k = int(topk * base_cfg.refine_ratio.value());
        base_cfg.k = refine_k;
        auto fp32_ds = ConvertToFp32DataSet<DataType>(dataset, is_cosine_);  
        auto quant_res = Search(fp32_ds, cfg, bitset);
        ...
        refine_offset_index_->SearchWithIds(nq, dataset->GetTensor(), queries_lims.data(), refine_ids, topk, distances, labels);
    ...
    RangeSearch(const DataSetPtr dataset, std::unique_ptr<Config> cfg, const BitsetView& bitset) const override {
        ...
        MatchQuantIndexConfig(cfg.get());
        auto fp32_ds = ConvertToFp32DataSet<DataType>(dataset, is_cosine_);  
        auto quant_res = RangeSearch(fp32_ds, cfg, bitset);
        if (!quant_res.has_value()) {
            return quant_res;
        }
        const BaseOffsetIndexRefineConfig& base_cfg = static_cast<const BaseOffsetIndexRefineConfig&>(*cfg);
        auto nq = dataset->GetRows();
        auto radius = base_cfg.radius;
        auto range_filter = base_cfg.range_filter;
        auto quant_res_ids = quant_res.value()->GetIds();
        auto quant_res_lims = quant_res.value()->GetLims();
        auto final_res = refine_offset_index_->RangeSearchWithIds(nq, dataset->GetTensor(), quant_res_lims,quant_res_ids, radius, range_filter);
        return final_res;
    }
    ...
    AnnIterator(const DataSetPtr dataset, std::unique_ptr<Config> cfg, const BitsetView& bitset) const override {
        ...
        auto iterator_refine_ratio = base_cfg.iterator_refine_ratio.value(); 
        auto fp32_ds = ConvertToFp32DataSet<DataType>(dataset, is_cosine_);  
        auto quant_init = AnnIterator(fp32_ds, cfg,bitset);
        ...
        for (auto i = 0; i < rows; i++) {
            ...
            vec[i] = std::shared_ptr(<IndexNode::IteratorPtr)(new iterator(std::move(quant_workspace_vec[i]), std::move(copied_query),larger_is_closer, iterator_refine_ratio));
        }
        return vec;
    }
    ...
    StaticCreateConfig() {
        if constexpr (std::is_same_v<faiss::IndexScaNN>>) {
            return std::make_unique<OffsetIndexSCANNConfig>();
        } else {
            return std::make_unique<BaseOffsetIndexRefineConfig>();
        }
    };
    ...
    Dim() const override {
        ...
        return quant_index_->Dim();
    }

    int64_t
    Size() const override {
        if (!quant_index_) {
            return quant_index_->Size();
        }
        return 0;
    }
    ...
    std::string
    Type() const override {
        if constexpr (std::is_same_v<fp32, faiss::IndexScaNN>) {
 ...
 private:
    class iterator : public IndexIterator {
        public:
        iterator(IteratorPtr quant_workspace,  std::unique_ptr<void*>&& copied_query,  bool larger_is_closer, const float refine_ratio = 0.5f) : 
                  IndexIterator(larger_is_closer, refine_ratio),
                  copied_query_(std::move(copied_query)),
                  quant_workspace_(quant_workspace) {
            refine_computer_ = select_data_view_computer(refine_offset_index_.get());
        ...
        raw_distance(int64_t id) override {
            ...
            return refine_computer->operator(id);
        }
        private:
        IteratorPtr quant_workspace_ = nullptr;
        std::unique_ptr<faiss::DistanceComputer> refine_computer_ = nullptr;
        std::unique_ptr<void*> copied_query_ = nullptr;
    };
    bool is_cosine_;  
    const ViewDataOp& view_data_op_; 
    std::shared_ptr<DataViewIndexFlat> refine_offset_index_; // a offset flat index to maintain raw data without extra memory
    std::shared_ptr<IndexNode> quant_index_;  // quant_index will hold codes in memory, datatype is fp32
};
}  // namespace knowhere