...
class MinHashLSH {
 public:
    ...
    Status
    BatchSearch(const char* query, size_t nq, float* distances, idx_t* labels, std::shared_ptr<ThreadPool> pool, MinHashLSHSearchParams* params);
    size_t
    Size() {
...
MinHashLSH::BuildAndSave(MinHashLSHBuildParams* params) {
    ...
    // load raw data, generate hash kv for each band and save raw data
    {
        ...
        if (bin_vec_dim != mh_vec_element_size * mh_vec_length * 8) {
            LOG_KNOWHERE_ERROR_ << "fail to load binary file, dim in file("<<bin_vec_dim<<") not equal to mh_vec_element_size * mh_vec_length * 8:"<<params->mh_vec_element_size * params->mh_vec_length * 8;
            return Status::disk_file_error;
        }
...
MinHashLSH::Search(const char* query, float* distances, idx_t* labels, MinHashLSHSearchParams* params) {
    ...
    if (search_with_jaccard) {
        ...
        res = std::shared_ptr<MinHashLSHResultHandler>(new MinHashLSHResultHandler(reorder_ids.get(),reorder_dis.get(), refine_k));  
    } else {
    ...
    if (search_with_jaccard) {
        minhash_jaccard_knn_ny_by_ids(query, this->raw_data_, reorder_ids.get(), this->mh_vec_length_, this->mh_vec_elememt_size_, res->topk_, topk, distances, labels); 
    }
    return Status::success;
}
Status
MinHashLSH::BatchSearch(const char* query, size_t nq, float* distances, idx_t* labels,
                                        std::shared_ptr<ThreadPool> pool, MinHashLSHSearchParams* params) {
    ...
    if (search_with_jaccard) {
        ...
        for (size_t i = 0; i < nq; i++) {
            all_res.emplace_back(reorder_ids_list.get() + i * refine_k,reorder_dis_list.get() + i * refine_k, refine_k);
        }
    } else {
        all_res.reserve(nq);
        for (size_t i = 0; i < nq; i++) {
    ...
    while(access_list.size() && band_ofs < this->band_) {
            ...
            futures.emplace_back(
                pool->push([&, beg = row * kQueryBatch, end = std::min((size_t)((row + 1) * kQueryBatch), access_num),
                    ...
                    for (auto index = beg; index < end; index++) {
                        ...
                        search_bands_list(query + mh_vec_elememt_size_ * mh_vec_length_ * query_id, beg, end, &all_res[query_id]);
                    }
                }));
        }
        WaitAllSuccess(futures);
        ...
        for (auto q_i : access_list) {
            if (!all_res[q_i].full()) {new_access_list.emplace_back(q_i);}
        }
    ...
    if (search_with_jaccard) {
        ...
        for (int64_t i = 0; i < nq; i++) {
            futures.emplace_back(pool->push([&, id = i](){
                ...
                minhash_jaccard_knn_ny_by_ids(q , this->raw_data_, reorder_ids, this->mh_vec_length_, this->mh_vec_elememt_size_, refine_k, topk, res_dis, res_ids); 
                return;
        }));
        WaitAllSuccess(futures);
    }
