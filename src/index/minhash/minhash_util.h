...
using DIST4FUNC = void (*)(const char* x, const char* y0, const char* y1,const char* y2,const char* y3,size_t size, size_t element_size, float& dis0, float& dis1, float& dis2, float& dis3);
inline float
...
inline void
minhash_jaccard_batch_4_native(const char* x, const char* y0, const char* y1,const char* y2,const char* y3,size_t size, size_t element_size, float& dis0, float& dis1, float& dis2, float& dis3) {
...
struct MinHashLSHResultHandler {
    ...
    size_t topk_;
    size_t counter_;
    MinHashLSHResultHandler(idx_t* res_ids, float* res_dis, size_t topk): ids_list_(res_ids),dis_list_(res_dis),topk_(topk) {
        counter_ = 0;
    ...
    bool find(const idx_t id) {
        return  (id != -1) && std::find(ids_list_, ids_list_ + topk_, id) != ids_list_ + topk_;
    }
    void push(const idx_t id, const float dis)  {
        if (id == -1 || dis < 0.000001f) return;
        if (topk_ > 1 && find(id)) {
            return;
        }
...
static inline void
minhash_lsh_hit_ny(const char* x, const char* y, size_t dim, size_t band, size_t ny, size_t topk, const BitsetView& bitset, float* vals, int64_t* ids) {
...
static inline void
minhash_jaccard_knn_ny(const char* x, const char* y, size_t length, size_t element_size, size_t ny, size_t topk, const BitsetView& bitset, float* vals, int64_t* ids) {
    // init
    ...
    auto filter =
    [&](const size_t j) { return (bitset.empty() || !bitset.test(j)); };
...
static inline void 
minhash_jaccard_knn_ny_by_ids(const char* x, const char* y, const int64_t* sel_ids, size_t length, size_t element_size, size_t sel_ids_num, size_t topk, float* res_vals, int64_t* res_ids) {
    // init
    for (size_t i = 0; i < topk; i++) {
        res_vals[i] = 0.0;
        res_ids[i] = -1;
