            ...
            switch (faiss_metric_type) {
                ...
                case faiss::METRIC_INNER_PRODUCT: {
                    ...
                    if (is_cosine) {
                        ...
                        faiss::knn_cosine(copied_query.get(), (const float*)xb, norms.get(), dim, 1, nb, &buf, id_selector);
                    } else {
        ...
        futs.emplace_back(pool->push([&, index = i] {
            ...
            switch (faiss_metric_type) {
                ...
                case faiss::METRIC_INNER_PRODUCT: {
                    ...
                    if (is_cosine) {
                        auto copied_query = CopyAndNormalizeVecs(cur_query, 1, dim);
                        faiss::range_search_cosine(copied_query.get(), (const float*)xb, norms.get(), dim, 1, nb, radius,
                                                   &res, id_selector);
                    } else {
