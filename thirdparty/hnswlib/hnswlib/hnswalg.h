...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
    ...
    std::unique_ptr<uint8_t []> index_memory_ = nullptr;
    ...
    void
    loadIndex(std::unique_ptr<uint8_t []>&& index_seq, size_t seq_size, size_t max_elements_i = 0) {
        // linxj: init with metrictype
        index_memory_ = std::move(index_seq);
        knowhere::MemoryPin input(index_memory_.get(), seq_size);
