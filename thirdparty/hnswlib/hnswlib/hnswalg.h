    ...
    getIteratorWorkspace(const void* query_data, const size_t seed_ef, const bool for_tuning,
        ...
        if constexpr (knowhere::KnowhereFloatTypeCheck<data_t>::value) {
            if (metric_type_ == Metric::COSINE) {
                ...
                knowhere::NormalizeVec((data_t*)normalized_query.get(), *(size_t*)dist_func_param_);
                return std::make_unique<IteratorWorkspace>(query_data, max_elements_, seed_ef, for_tuning,
                                                           std::move(normalized_query), bitset, accumulative_alpha);
            }
        }
